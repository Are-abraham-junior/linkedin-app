 ...
 */
export async function syncUserChatsWithUnipile(userId: string, limit: number = 50, force: boolean = false) {
  const now = Date.now();
  ...
  for (const item of unipileChats.items) {
    ...
    let otherAttendee =
      attendees.find((a: any) => !a.is_self && (a.provider_id || a.id || a.name)) ||
      attendees[0] ||
      {};
    ...
    let attendeeHeadline =
      otherAttendee.headline || otherAttendee.occupation || "";
    ...
    if (existingConvForChat?.prospect && existingConvForChat.prospect.firstName && existingConvForChat.prospect.firstName !== "Contact") {
      ...
      if (!attendeePicture) attendeePicture = existingConvForChat.prospect.avatarUrl;
      if (!attendeeHeadline) attendeeHeadline = existingConvForChat.prospect.headline || "";
      if (!profileUrl) profileUrl = existingConvForChat.prospect.linkedinUrl;
    }
    ...
    if (!attendeeName || attendeeName === "Contact LinkedIn" || attendeeName === "LinkedIn Member") {
      try {
        ...
        if (attendeesRes.success && Array.isArray(attendeesRes.items) && attendeesRes.items.length > 0) {
          const realOther =
            attendeesRes.items.find((a: any) => !a.is_self && (a.name || a.provider_id)) ||
          ...
          if (realOther) {
            ...
            if (realOther.picture_url) attendeePicture = realOther.picture_url;
            if (realOther.specifics?.occupation) attendeeHeadline = realOther.specifics.occupation;
            if (realOther.provider_id) providerId = realOther.provider_id;
            if (realOther.profile_url) profileUrl = realOther.profile_url;
          }
        }
      } catch (attErr) {
    ...
    if (!prospect) {
      ...
      const updates: any = {};
      if (!(prospect as any).userId) updates.userId = userId;
      ...
      if (profileUrl && !prospect.linkedinUrl.includes("/in/")) updates.linkedinUrl = profileUrl;
    ...
    if (existingConv) {
      ...
      const finalUnread =
        existingConv.unreadCount === 0 && !isNewerMessage ? 0 : unreadCount;
