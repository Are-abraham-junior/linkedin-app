...
function readAvatar(raw: any): string | null {
  return raw && typeof raw === "object" && typeof raw.avatarUrl === "string" ? raw.avatarUrl : null;
}

export async function getWorkspaceAvatar(organizationId: string): Promise<string | null> {
...
export async function getWorkspaceAvatars(organizationIds: string[]): Promise<Map<string, string | null>> {
  ...
  for (const r of rows) map.set(r.organizationId, readAvatar(r.config));
  return map;
}

export async function setWorkspaceAvatar(organizationId: string, avatarUrl: string | null): Promise<void> {
