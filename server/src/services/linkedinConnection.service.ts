...
interface ConnectParams {
  email: string;
  password: string;
  /** account_id Unipile déjà connu pour cet utilisateur (même DISCONNECTED / CHECKPOINT). */
 ...
 */
export async function connectOrReconnect(params: ConnectParams): Promise<ConnectOutcome> {
  const email = params.email.trim();
  let reused = false;
  let result: Awaited<ReturnType<typeof UnipileService.connectLinkedInAccount>> & { notFound?: boolean };
 ...
 */
export async function finalizeConnection(accountId: string, reused = false): Promise<ConnectOutcome> {
  ...
  const removedDuplicates = await UnipileService.dedupeUnipileAccounts(accountId, profile?.linkedinProfileId).catch(
    (err: any) => {
 ...
 */
export async function persistLinkedInAccount(
  userId: string,
  accountId: string,
  profile: any,
  status: "CONNECTED" | "CHECKPOINT",
 ...
 */
export async function pendingCheckpointFor(userId: string): Promise<{ accountId: string; checkpoint: any } | null> {
...
export async function resumePausedCampaigns(userId: string) {
