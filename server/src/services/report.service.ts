...
export interface ReportSummary extends ActionCounters {
  campaigns: number;
...
export interface BuildReportOptions {
  /** Clause `where` Prisma sur Campaign (périmètre déjà résolu). */
  scope: any;
  from: Date;
  to: Date;
  campaignIds?: string[];
  includeDetails?: boolean;
  /** Calcule aussi les compteurs de la période précédente de même durée. */
  compare?: boolean;
}

export function emptyCounters(): ActionCounters {
...
interface ActionAggregation {
  counters: Map<string, ActionCounters>;
  timeline: Map<string, Map<string, TimelinePoint>>;
}
 ...
 */
async function aggregateActions(ids: string[], from: Date, to: Date, withQueued: boolean): Promise<ActionAggregation> {
  const counters = new Map<string, ActionCounters>();
  const timeline = new Map<string, Map<string, TimelinePoint>>();
  if (ids.length === 0) return { counters, timeline };
  ...
  if (withQueued) or.push({ status: "QUEUED" });
  ...
  const bump = (campaignId: string, day: string, key: keyof Omit<TimelinePoint, "date">) => {
  ...
  for (const a of actions) {
    ...
    switch (a.actionType) {
      case "INVITATION":
        c.invitesSent += 1;
        if (day) bump(a.campaignId, day, "invitesSent");
        break;
      case "MESSAGE":
        c.messagesSent += 1;
        if (day) bump(a.campaignId, day, "messagesSent");
        break;
      case "VISIT_PROFILE":
        c.visits += 1;
        break;
...
export async function buildCampaignsReport(opts: BuildReportOptions) {
  const { scope, from, to, includeDetails = false, compare = false } = opts;
  const now = new Date();

  const where: any = { ...scope };
  if (opts.campaignIds && opts.campaignIds.length > 0) where.id = { in: opts.campaignIds };
  ...
  const [current, previous] = await Promise.all([
    ...
    compare ? aggregateActions(ids, prevFrom, prevTo, false) : Promise.resolve<ActionAggregation | null>(null),
  ]);
  ...
  const formatted = campaigns.map((c: any) => {
    const states: any[] = c.prospectStates || [];
    ...
    for (const k of Object.keys(counters) as (keyof ActionCounters)[]) {
      summary[k] += counters[k];
      if (previousSummary && prevCounters) previousSummary[k] += prevCounters[k];
    }
  ...
  const comparison = previousSummary
    ? {
        ...
        deltas: Object.fromEntries(
          (Object.keys(previousSummary) as (keyof ActionCounters)[]).map((k) => [k, delta(summary[k], previousSummary[k])])
        ) as Record<keyof ActionCounters, number | null>,
      }
    : null;
...
export type CampaignsReport = Awaited<ReturnType<typeof buildCampaignsReport>>;
