...
export async function searchProfiles(req, res) {
    try {
        ...
        const isExpiredOrNotFound = status === 404 ||
            status === 401 ||
...
export async function getSearchParameters(req, res) {
    try {
        const userId = req.user.id;
        const { type, keywords, service, limit = "20" } = req.query;
        if (!type) {
            res.status(400).json({
                success: false,
...
export async function getAccountHealth(req, res) {
    try {
        ...
        try {
            const status = await UnipileService.getAccountStatus(linkedAcc.unipileAccountId);
            res.json({
                success: true,
                status,
            ...
            const isNotFoundOrExpired = errorMsg.includes("404") ||
                errorMsg.includes("401") ||
                errorMsg.toLowerCase().includes("not found") ||
                errorMsg.toLowerCase().includes("not_found") ||
                errorMsg.toLowerCase().includes("unauthorized");
