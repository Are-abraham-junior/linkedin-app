...
import type { CampaignReport, ReportCampaign, ReportTimelinePoint } from "../types";
...
/** Agrège les timelines de toutes les campagnes en une série jour par jour. */
export function mergeTimelines(campaigns: ReportCampaign[]): ReportTimelinePoint[] {
  const byDay = new Map<string, ReportTimelinePoint>();
...
/** Libellé de variation « +12 % » / « -5 % » / « — » (ASCII : Helvetica du PDF n'a pas le glyphe U+2212). */
export function fmtDelta(d: number | null | undefined): string {
  if (d === null || d === undefined) return "—";
  return `${d > 0 ? "+" : d < 0 ? "-" : ""}${Math.abs(d)} %`;
}

export function exportReportToExcel(report: CampaignReport, includeProspects: boolean) {
  ...
  XLSX.writeFile(wb, `bleadin-rapport-${fileStamp()}.xlsx`);
}
...
/** Complète la série avec les jours sans activité (0) entre `from` et `to` pour une courbe continue. */
export function fillTimeline(points: ReportTimelinePoint[], from: string, to: string): ReportTimelinePoint[] {
  ...
  // Garde-fou : au-delà d'un an on ne remplit pas (la courbe reste lisible)
...
function drawTimelineChart(doc: jsPDF, y: number, points: ReportTimelinePoint[]): number {
  const h = 55;
  const chartTop = y + 6;
  ...
  const series: Array<{ key: keyof Omit<ReportTimelinePoint, "date">; color: [number, number, number] }> = [
  ...
  // Étiquettes de date (sous-ensemble si trop de points)
  ...
  // Aire légère sous chaque courbe, puis la courbe et ses points
  for (const s of series) {
    const coords = points.map((p, i) => [xAt(i), yAt(p[s.key])] as [number, number]);
    if (points.length > 1) {
      // Aire : polygone fermé sur l'axe X, très transparent
...
export function exportReportToPdf(report: CampaignReport, includeProspects: boolean) {
  ...
  doc.save(`bleadin-rapport-${fileStamp()}.pdf`);
}

// ─── Contacts (prospects avec e-mail ou téléphone) ───────────────────────────
...
/**
 * Exporte en Excel les prospects d'une ou plusieurs campagnes disposant d'un
 * e-mail ou d'un numéro de téléphone. Renvoie le nombre de contacts exportés
 * et le nom du fichier (0 → aucun fichier généré).
 */
export function exportContactsToExcel(campaigns: ReportCampaign[]): { count: number; filename: string } {
  const rows = campaigns.flatMap((c) =>
    (c.prospects || [])
      .filter((p) => (p.email && p.email.trim()) || (p.phone && p.phone.trim()))
      .map((p) => ({
        Prénom: p.firstName,
        Nom: p.lastName,
        Email: p.email || "",
  ...
  const suffix = campaigns.length === 1 ? slugify(campaigns[0].name) : "campagnes";
  const filename = `bleadin-contacts-${suffix}-${fileStamp()}.xlsx`;
  if (rows.length === 0) return { count: 0, filename };

  const wb = XLSX.utils.book_new();
  addSheet(wb, "Contacts", rows);
