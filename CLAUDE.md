...
- Automatic e-mail reports: per-user prefs are real `User` columns (`reportEmailFrequency`, `reportEmails[]`, `reportHour`, `reportDay`, `reportLastSentAt`, interpreted in `User.timezone`), driven by `server/src/workers/report.worker.ts` (15-min tick, at most one send per local day). Export history still lives in the `IntegrationConfig` JSON blob with `provider = BLEADIN_REPORTS` (`reportSettings.service.ts`); a one-shot `migrateLegacyReportPrefs()` at scheduler start moves any leftover `config.users` prefs to `User`.
