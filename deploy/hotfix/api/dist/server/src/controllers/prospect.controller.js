...
export async function bulkImportProspects(req, res) {
    try {
        ...
        if (!list) {
            ...
            console.warn(`[bulkImportProspects] Liste refusée — listId=${body.listId} existe=${Boolean(existing)} ` +
        ...
        try {
            await prisma.importHistory.create({
                data: {
                    ...
                    listId: body.listId,
                    listName: list.name,
                    source: body.source || (body.prospects[0]?.providerProfileId ? "LINKEDIN_SEARCH" : "FICHIER"),
                    filename: body.filename || `Import_${list.name}_${new Date().toLocaleDateString("fr-FR").replace(/\//g, "-")}`,
