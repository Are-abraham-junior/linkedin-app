...
import { getReportsConfig, getUserPrefs, setUserPrefs, appendHistory, } from "../services/reportSettings.service.js";
 ...
 */
export async function getCampaignsReport(req, res) {
    try {
...
export async function sendReportNow(req, res) {
    try {
        const { organizationId } = await resolveOrganization(req);
        const config = organizationId ? await getReportsConfig(organizationId) : null;
        const prefs = config ? getUserPrefs(config, req.user.id) : { emailFrequency: "NONE" };
        const requested = req.body?.frequency;
        const frequency = requested === "DAILY" || requested === "WEEKLY"
            ? requested
...
export async function addReportHistory(req, res) {
    try {
