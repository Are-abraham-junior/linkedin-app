...
import { connectOrReconnect, finalizeConnection, persistLinkedInAccount, pendingCheckpointFor, resumePausedCampaigns, } from "../services/linkedinConnection.service.js";
...
export async function updateAccountSettings(req, res) {
    try {
        ...
        if (body.maxDailyInvites !== undefined)
            updateData.maxDailyInvites = body.maxDailyInvites;
        if (body.maxDailyMsg !== undefined)
            updateData.maxDailyMsg = body.maxDailyMsg;
...
// ==========================================
export async function getLinkedInSettings(req, res) {
    try {
        const userId = req.user.id;
...
export async function reconnectLinkedInDirect(req, res) {
    try {
        const userId = req.user.id;
...
export async function resolveLinkedInCheckpoint(req, res) {
    try {
        const userId = req.user.id;
...
export async function resendLinkedInCheckpoint(req, res) {
    try {
    ...
    try {
        ...
        if (provider === "BREVO") {
            ...
            try {
                ...
                if (brevoRes.ok) {
                    const accData = await brevoRes.json();
                    res.json({
                        success: true,
