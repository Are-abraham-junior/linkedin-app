...
import { generateToken, JWT_SECRET } from "../middlewares/auth.middleware.js";
import { sendPasswordResetEmail } from "../services/mail.service.js";
import { connectOrReconnect, finalizeConnection, persistLinkedInAccount, pendingCheckpointFor, } from "../services/linkedinConnection.service.js";
...
function authenticatedUserIdFrom(req) {
 ...
 */
async function findUserForLinkedInLogin(req, linkedinEmail) {
 ...
 */
async function completeLinkedInAuth(req, res, outcome, linkedinEmail) {
    const profile = outcome.profile;
    let user = (await findUserForLinkedInLogin(req, linkedinEmail)).user;
    if (!user) {
        user = await prisma.user.findFirst({
            where: {
                OR: [
    ...
    if (user) {
        ...
        if (!user.linkedinEmail)
            updateData.linkedinEmail = linkedinEmail;
...
export async function linkedinAuth(req, res) {
    try {
        ...
        const existingAccountId = preUser?.accounts?.find((a) => a.unipileAccountId)?.unipileAccountId || null;
 ...
 */
export async function linkedinAuthCheckpoint(req, res) {
    try {
...
export async function linkedinAuthResendCheckpoint(req, res) {
...
export async function forgotPassword(req, res) {
    try {
        ...
        const token = jwt.sign({ sub: user.id, purpose: RESET_TOKEN_PURPOSE, pwh: passwordFingerprint(user.passwordHash) }, JWT_SECRET, { expiresIn: "1h" });
