...
function deltaBadge(delta) {
    if (delta === null)
        return `<span style="font-size:11px;color:#9a9aa5;">—</span>`;
    const up = delta >= 0;
 ...
 */
export async function sendCampaignReportEmail(params) {
    const from = process.env.SMTP_FROM || "Bleadin <no-reply@bleadin.com>";
    ...
    const text = [
        ...
        "",
        ...params.campaigns.map((c) => `- ${c.name} [${c.status}] : ${c.totalProspects} prospects, ${c.invitesSent} invitations, ${c.acceptanceRate} % acceptées, ${c.messagesSent} messages, ${c.repliesReceived} réponses (${c.replyRate} %)`),
        "",
