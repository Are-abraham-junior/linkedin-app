 ...
 */
function unipileFetch(url, options) {
...
export class UnipileService {
    ...
    static parseErrorResponse(status, errText) {
        if (status === 502) {
    ...
    static async safeJsonParse(res) {
     ...
     */
    static async findAccountsByProviderId(providerId) {
        if (!providerId)
            return [];
     ...
     */
    static async dedupeUnipileAccounts(keepAccountId, providerId) {
        if (!keepAccountId || !providerId)
            return [];
     ...
     */
    static async connectLinkedInAccount(linkedinEmail, linkedinPassword) {
        try {
            const baseUrl = this.getBaseUrl();
            const res = await unipileFetch(`${baseUrl}/api/v1/accounts`, {
                method: "POST",
     ...
     */
    static async reconnectLinkedInAccount(accountId, linkedinEmail, linkedinPassword) {
        try {
            const baseUrl = this.getBaseUrl();
            const res = await unipileFetch(`${baseUrl}/api/v1/accounts/${accountId}`, {
                method: "POST",
     ...
     */
    static async solveCheckpoint(accountId, code) {
        try {
            const baseUrl = this.getBaseUrl();
     ...
     */
    static async resendCheckpoint(accountId) {
        try {
            const baseUrl = this.getBaseUrl();
     ...
     */
    static async detectLinkedInSubscription(accountId) {
        let isPremium = false;
        ...
        try {
            ...
            try {
                ...
                if (meRes.ok) {
                    ...
                    if (meData) {
                        isPremium = Boolean(meData?.is_premium ??
            ...
            try {
                ...
                if (accRes.ok) {
                    ...
                    if (productsStatus.sales_navigator === "running" ||
                        productsList.includes("sales_navigator")) {
                        hasSalesNavigator = true;
                    }
                    if (productsStatus.recruiter === "running" ||
     ...
     */
    static async getConnectedAccountProfile(accountId) {
        try {
            const baseUrl = this.getBaseUrl();
        ...
        try {
            ...
            while (accumulatedItems.length < targetLimit && iterations < maxIterations) {
                ...
                if (currentCursor) {
                    ...
                    const terms = [];
                    if (params.title)
                        terms.push(params.title.trim());
                    if (params.company)
                        terms.push(params.company.trim());
                    if (params.location)
                        terms.push(params.location.trim());
                ...
                for (const item of rawItems) {
                    ...
                    if (!accumulatedItems.some((existing) => existing.providerProfileId === profileId)) {
                        accumulatedItems.push({
                            ...
                            avatarUrl: item.profile_picture_url_large ||
     ...
     */
    static async getChatAttendees(chatId) {
        try {
     ...
     */
    static isOwnMessage(m) {
        if (!m)
            return false;
        const flag = m.is_sender;
        if (flag === 1 || flag === true || flag === "1" || flag === "true")
            return true;
        if (flag === 0 || flag === false || flag === "0" || flag === "false")
            return false;
     ...
     */
    static parseLinkedInConnectionStatus(item) {
        if (!item)
            return "NOT_CONNECTED";
        if (item.is_relationship === true || item.is_self === true) {
            return "CONNECTED";
        }
        const dist = String(item.network_distance || item.networkDistance || item.distance || "").toUpperCase();
        if (dist === "DISTANCE_1" ||
            dist === "FIRST_DEGREE" ||
            dist === "FIRST" ||
            dist === "1ST" ||
        ...
        if (item.pending_invitation === true ||
            dist === "PENDING" ||
