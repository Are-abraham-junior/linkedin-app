 ...
 */
export async function handleProspectReply(prospectId, text, targetUserId) {
    try {
...
export async function sendMessage(req, res) {
    try {
        ...
        const isServiceUnavailable = sendRes?.error?.includes("502") ||
        ...
        if (!targetChatId || !sendRes?.success) {
            ...
            if (!foundChatId) {
                ...
                if (startRes.success && startRes.chatId) {
                    ...
                    sendRes = {
                        success: true,
                        messageId: startRes.messageId || startRes.id || "init_chat_msg",
                    };
                }
                else {
                    res.status(500).json({
