...
export function extractCompany(headline) {
  ...
  if (parts.length > 1) {
    for (let i = 0; i < parts.length; i++) {
      ...
      const brandMatch = p.match(/\b(CFAO|Orange|MTN|Moov|Société Générale|Ecobank|Total|Shell|Bolloré|Spiro|Studely|Canal\+|Yango|Wave|Lapaire|AIB|Coris|ESSEG|NETIS)\b(?:\s+[A-Za-z0-9À-ÿ+\-]+)*/i);
      if (brandMatch) {
...
async function run() {
  ...
  for (const p of prospects) {
    if (!p.company || p.company === "—" || p.company === "null") {
      const detected = extractCompany(p.headline);
      console.log(`[${p.firstName} ${p.lastName}] headline: "${p.headline}" -> detected company: "${detected || "(none)"}"`);
      if (detected) {
      ...
      console.log(`[${p.firstName} ${p.lastName}] ALREADY HAS COMPANY: "${p.company}"`);
    }
  }

  console.log(`Updated ${updated} prospects with company!`);
  process.exit(0);
}
